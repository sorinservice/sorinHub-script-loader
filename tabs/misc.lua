...
return function(tab, OrionLib)
    local CATEGORY_KEYWORDS = { "Verschiedene", "Misc", "Various" }
    ...
    local function parseNumber(txt)
        return tonumber((tostring(txt or "")):gsub("[^%d]", ""))
    end
            ...
            if row:IsA("Frame") then
                ...
                if name and val and name:IsA("TextLabel") and val:IsA("TextLabel") then
                    table.insert(items, {
                        ...
                        colorHex= colorToHex(getRowTint(row) or Color3.new(1,1,1)),
                    })
                end
            end
        end
    ...
    sections.summary:AddButton({ Name="Scan Boards", Callback=function() refresh(); OrionLib:MakeNotification({Name="Scan", Content="Misc scan completed.", Time=3}) end })
