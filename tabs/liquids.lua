...
return function(tab, OrionLib)
    local CATEGORY_KEYWORDS = { "Flüssig", "Liquid", "Liquids" } -- matches "Flüssige Verkaufswerte"
    ...
    local function containsKeywordAny(s, keys)
        ...
        for _, k in ipairs(keys) do
            if string.find(s, string.lower(k)) then return true end
        end
        return false
    end
    ...
    sections.summary:AddButton({ Name="Scan Boards", Callback=function() refreshOnce(); OrionLib:MakeNotification({Name="Scan", Content="Liquids scan completed.", Time=3}) end })
